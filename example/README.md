...
Without `NORIA_DEMO_SESSION_ENDPOINT` or `NORIA_DEMO_CART_ID`, the app runs as a
non-interactive visual preview. Use a stable cart/order ID while the same order
is pending and a new ID for a genuinely new order. Never place the project's
Ed25519 private key or a Noria internal key in a `--dart-define` or in the
application bundle.
